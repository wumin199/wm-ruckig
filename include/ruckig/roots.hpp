...
这些函数都被封装在命名空间 ruckig::roots
中。这些函数可以用于多种应用，例如在机器人控制中计算运动规划中的时间或速度等。
*/
...
template <typename T, size_t N> class PositiveSet : public Set<T, N> {
public:
  void insert(T value) {
    if (value >= 0) {
...
possibly three. But unlike a quadratic equation which may have no real solution,
a cubic equation always has at least one real root. We will see why this is the
case later. If a cubic does have three roots, two or even all three of them may
be repeated. This gives us four possibilities which are illustrated in the
following examples.
...
不是完全理解他的解法：https://www.1728.org/cubic2.htm
...
inline PositiveSet<double, 3> solveCub(double a, double b, double c, double d) {
  ...
  if (std::abs(d) < DBL_EPSILON) {
    ...
    b = a;
    a = 0.0;
    //   a*x^3 +   b*x^2 +   c*x +  d = 0， 如果 d =0，则
    //   a*x^3 +   b*x^2 +   c*x +  0 = 0  -> 同时 /x
    //             a*x^2 +   b*x +  c  = 0  -> -
    //  归一化到下面这种情况
    // (0)*x^3 + (a)*x^2 + (b)*x + (c) = 0
  }
  ...
  if (std::abs(a) < DBL_EPSILON) {
    ...
    const double halfq =
        (2 * bb * b - 9 * a * b * c + 27 * a * a * d) / 54 * invaa * inva;
    ...
    if (yy > DBL_EPSILON) {
      ...
      const double y = std::sqrt(yy); // y = sqrt(delta)
      const double uuu = -halfq + y;  // u^3
      const double vvv = -halfq - y;  // v^3
      const double www = std::abs(uuu) > std::abs(vvv) ? uuu : vvv;
      const double w = std::cbrt(www); // 是计算立方根
...
inline int solveResolvent(std::array<double, 3> &x, double a, double b,
                          double c) {
  constexpr double cos120 = -0.50;
  constexpr double sin120 = 0.866025403784438646764;
...
//! Calculate all roots of the monic quartic equation: x^4 + a*x^3 + b*x^2 + c*x
//! + d = 0
inline PositiveSet<double, 4> solveQuartMonic(double a, double b, double c,
                                              double d) {
...
inline PositiveSet<double, 4>
solveQuartMonic(const std::array<double, 4> &polynom) {
...
polyMonicDeri(const std::array<double, N> &monic_coeffs) {
  std::array<double, N - 1> deriv;
  deriv[0] = 1.0;
...
inline double shrinkInterval(const std::array<double, N> &p, double l,
                             double h) {
